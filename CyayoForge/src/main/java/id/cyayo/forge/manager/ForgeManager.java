...
public class ForgeManager {
    ...
    public ItemStack buildOutput(ForgeSession session) {
        try {
            ...
            return injectMaterialData(item, session.getMaterials());
        } catch (Exception e) {
    ...
    private void applyDoubleStatMult(MMOItem mmoItem, ItemStat stat, double multiplier) {
        ...
        DoubleData d = (DoubleData) mmoItem.getData(stat);
        if (d != null) mmoItem.setData(stat, new DoubleData(d.getValue() * multiplier));
    }
        ...
        for (Map.Entry<String, Double> e : mergedStats.entrySet()) {
            try {
                ...
                if (mmoItem.hasData(stat)) {
                    ...
                    if (existing != null)
                        mmoItem.setData(stat, new DoubleData(existing.getValue() + e.getValue()));
                } else {
                    mmoItem.setData(stat, new DoubleData(e.getValue()));
                }
            } catch (Exception ex) {
    ...
    private ItemStack injectMaterialData(ItemStack item, List<ItemStack> materials) {
        if (item == null || materials == null || materials.isEmpty()) return item;
        try {
            ...
            for (ItemStack mat : materials) {
                ...
                // Simpan display name untuk keperluan visual di GUI salvage nanti
            ...
            NBTItem nbt = NBTItem.get(item);
            // Mencoba menggunakan varargs dengan eksplisit array jika diperlukan, atau ganti ke getString check
            nbt.addTag(new io.lumine.mythic.lib.api.item.ItemTag("CYAYOFORGE_MATERIALS", json));
            return nbt.toItem();
        } catch (Exception e) {
            plugin.getLogger().log(Level.WARNING, "Gagal inject data material ke NBT", e);
            return item;
        }
    }
